    ...
    with tempfile.TemporaryDirectory() as folder:
        ...
        for index, cue in enumerate(cues):
            ...
            parts.extend([
                ...
                *character_voice_filters(cue.get('tag', 'M')),
...
with tab_video:
    ...
    # Source transcription is always downloadable, even when Gemini translation fails.
    if st.session_state.get("source_srt_text", "").strip():
        st.download_button(
            "⬇️ ទាញ Source SRT ពីវីដេអូ",
            ("\ufeff" + st.session_state.source_srt_text).encode("utf-8"),
            f"{safe_download_stem(st.session_state.get('source_video_stem'), 'video')}_source.srt",
            "application/x-subrip",
            key="download_source_srt_video",
            use_container_width=True,
        )
