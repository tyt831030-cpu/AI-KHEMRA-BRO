...
def _public_app_url():
    """Return the clean public app URL used when Admin creates customer links."""
    try:
        configured = str(st.secrets.get("PUBLIC_APP_URL", "")).strip().rstrip("/")
    except Exception:
        configured = ""
    if configured:
        return configured
    try:
        current = str(st.context.url).split("?", 1)[0].rstrip("/")
...
def _customer_link(code):
    from urllib.parse import quote
    return f"{_public_app_url()}/?code={quote(_normalize_access_code(code))}"
...
def render_admin_panel():
    ...
    if st.session_state.get("last_created_code"):
        created_code = st.session_state.last_created_code
        customer_link = _customer_link(created_code)
        st.success("✅ Access Code និងលីងអតិថិជនរួចរាល់")
        st.text_input("Access Code", value=created_code, disabled=True, key="created_code_display")
        st.text_area(
            "🔗 លីងផ្ញើទៅអតិថិជន",
            value=customer_link,
            height=92,
            disabled=True,
            key="created_link_display",
        )
        st.caption("អតិថិជនគ្រាន់តែចុចលីងនេះ។ កូដនឹងត្រូវបញ្ចូល និងពិនិត្យដោយស្វ័យប្រវត្តិ។")
    ...
    for row in rows:
        ...
        with st.expander(f'{row["code"]} • {status}'):
            ...
            st.text_input(
                "លីងអតិថិជន",
                value=_customer_link(row["code"]),
                disabled=True,
                key=f'link_{row["code"]}',
            )
            ...
            with c3:
                if st.button("🗑️ លុប", key=f'delete_{row["code"]}', use_container_width=True):
                    with _db_connection() as db:
                        ...
                        db.commit()
                    st.rerun()
    st.caption("ផ្ទាំង Admin មិនបង្ហាញលើទំព័រអតិថិជនទេ។ រក្សាទុកលីង Admin របស់អ្នកជាសម្ងាត់។")
    st.stop()
...
def require_login():
    ...
    # Customer links generated by Admin contain ?code=... . Validate once,
    # sign this browser in automatically, then remove the code from the URL.
    try:
        shared_code = _normalize_access_code(st.query_params.get("code", ""))
    except Exception:
        shared_code = ""
    if shared_code and not st.session_state.get("authenticated"):
        valid, message, row = _validate_access_code(shared_code, st.session_state.device_id)
        if valid:
            st.session_state.authenticated = True
            st.session_state.access_code = shared_code
            _save_auth_cookie(shared_code, st.session_state.device_id, row["expires_at"])
            try:
            ...
            st.session_state.shared_code_error = message
    ...
    if st.session_state.pop("shared_code_error", None):
        st.error("❌ លីងនេះមាន Access Code មិនត្រឹមត្រូវ ផុតកំណត់ ឬត្រូវបានបិទ។")
